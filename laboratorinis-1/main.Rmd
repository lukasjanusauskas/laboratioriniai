...
\end{equation}
 ...
 $\chi^2$ skirtinys buvo pirmą kartą atrastas Vokiečių statistiko Fredriko Roberto Helmerto 1875-6 metais, bet vėliau šį skirstinį nepriklausomai iš naujo atrado anglų matematikas Karlas Pirsonas, nagrinėdamas suderinamumo testą(angl. goodness of fit testas).
Kuriam jis sukūrė savo Pirsono $\chi^2$ testą(chi-squared test).

### Taikymas

Chi kvadrato skirstinys yra labai svarbus statistikoje, ypač hipotezių tikrinimui ir dispersijos analizei.

Researchers used the chi-square test to study Mendelian inheritance patterns, validating the laws of
inheritance proposed by Gregor Mende
...
Skirtumas tarp čių dviejų testų: nepriklausomumo testas tikrina, ar yra ryšys tarp dviejų kategorinių kintamųjų, o suderinanamumo - ar pasiskirstymas
...
\end{equation}
```
Kur $display="\Gamma(k/2)$ yra Oilerio gama funkcija.
...
Skaitinės charakteristikos ( [@kruopistextbook] 3.9 apibrėžimas )
...
```
Matome, kad didėjant imties dydžiui, historigrama vis tiksliau atitinka chi² skirstinį.
